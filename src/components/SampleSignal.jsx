...
export default function SampleSignal() {
  ...
  return (
    <section id="signal" className="container-x scroll-mt-24 py-24 sm:py-32">
      <div
        ref={ref}
        className="reveal relative overflow-hidden rounded-3xl border border-line bg-mist p-8 sm:p-12"
      >
        <div className="absolute -right-16 -top-16 h-56 w-56 rounded-full bg-accent/5 blur-3xl" />

        <div className="relative">
          <div className="flex flex-wrap items-center gap-3">
            <span className="eyebrow">{s.kicker}</span>
            <span className="text-ink-faint">·</span>
            <span className="text-xs font-medium text-accent">{s.region}</span>
            <span className="text-ink-faint">·</span>
            <span className="text-xs font-medium text-accent-soft">{s.theme}</span>
          </div>
